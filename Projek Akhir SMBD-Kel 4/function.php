...
function tambah_data_menu()
{
    ...
    // Generate Kode Menu
    $kode_menu = "MN" . (int)ambil_data("SELECT MAX(SUBSTR(kode_menu, 3)) AS kode FROM menu")[0]["kode"] + 1;
    ...
    $id_menu = (int)ambil_data("SELECT MAX(id_menu) AS id FROM menu")[0]["id"] + 1;

    mysqli_query($koneksi, "INSERT INTO menu (id_menu, kode_menu, nama, harga, gambar, kategori, status)
...
function tambah_data_pesanan()
{
    ...
    // Mengambil Data jumlah dan Kode Menu
    $list_pesanan = [];
    $max_menu = count(ambil_data("SELECT * FROM menu"));
    for ($i = 1; $i <= $max_menu; $i++) {
        if (isset($_POST["jumlah$i"]) && $_POST["jumlah$i"] > 0) {
            if (isset($_POST["id_menu$i"]) && isset($_POST["kode_menu$i"])) {
                ...
                echo "<script>alert('Data menu tidak lengkap!');</script>";
                return -1;
            }
        }
    }
    ...
    foreach ($list_pesanan as $lp) {
        ...
        // Periksa apakah data menu lengkap
        if (empty($id_menu) || empty($kode_menu) || $jumlah <= 0) {
            echo "<script>alert('Data menu tidak lengkap!');</script>";
            return -1;
        }

        // Tambahkan pesanan ke database
        mysqli_query($koneksi, "INSERT INTO pesanan (id_pelanggan, kode_pesanan, id_menu, kode_menu, jumlah)
                                VALUES ('$pelanggan', '$kode_pesanan', '$id_menu', '$kode_menu', '$jumlah')");
    }

    // Tambahkan data transaksi jika pesanan valid
    $id_pesanan = mysqli_insert_id($koneksi); // Mendapatkan ID pesanan yang baru saja dimasukkan
    mysqli_query($koneksi, "INSERT INTO transaksi (id_karyawan, id_pesanan, waktu)
                            VALUES (NULL, '$id_pesanan', NOW())");
...
function hapus_data_pesanan()
{
    ...
    // eksekusi query delete
    mysqli_query($koneksi, "DELETE FROM transaksi WHERE id_pesanan IN (SELECT id_pesanan FROM pesanan WHERE kode_pesanan = '$kode_pesanan')");
